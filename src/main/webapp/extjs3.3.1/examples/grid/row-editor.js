Ext.onReady(function(){
    ...
    var grid = new Ext.grid.GridPanel({
        ...
        tbar: [{
            ...
            handler: function(){
                ...
                editor.stopEditing();
                store.insert(0, e);						//插入第一行
                grid.getView().refresh();
                grid.getSelectionModel().selectRow(0);	//选中插入的新行
                editor.startEditing(0);					//处于编辑状态
            }
        },{
            ref: '../removeBtn',
            ...
            handler: function(){
                editor.stopEditing();
                var s = grid.getSelectionModel().getSelections();	//选中的行将被删除
                for(var i = 0, r; r = s[i]; i++){
                    store.remove(r);
                }
                grid.getView().refresh();				//刷新,表格前的Grid的行号刷新
            }
        }],

        columns: [
        new Ext.grid.RowNumberer(),			//Grid的行号
        {
            id: 'name',
            header: 'First Name',
            dataIndex: 'name',
    ...
    //Grid的数据发生变化时,触发cstore.refreshData方法,报表发生变化
    //Ext.data.GroupingStore的add,remove,update方法是其已经具有.
    cstore.refreshData();
