...
Ext.onReady(function(){
    ...
    //概要这一行最后一列求总和:record.data即被分组同一组下的每一行记录. v代表神马?
    ...
    var grid = new xg.EditorGridPanel({
        ds: new Ext.data.GroupingStore({
            reader: reader,						//use json reader
            data: app.grid.dummyData,			//use local data使用本地数据
            sortInfo: {field: 'due', direction: 'ASC'},
            groupField: 'project'				//按照project分组
        }),
        columns: [
            {
                id: 'description',
                ...
                hideable: false,
                summaryType: 'count',			//当前组概要这一行的对应列的显示summaryType(count计数,max最大值,sum求和) summaryRenderer概要的自定义呈现
                ...
                sortable: true,
                dataIndex: 'due',
                summaryType: 'max',				//概要类型:最大值.即当前组下当前列中值最大的.
                ...
                sortable: true,
                dataIndex: 'estimate',
                summaryType: 'sum',				//概要类型:求和.即当前组下当前列所有行的值求和.
                renderer : function(v){
                    return v +' hours';
                },
                ...
                dataIndex: 'rate',
                summaryType: 'average',			//概要类型:求平均值.同上,对当前组当前列所有行的值求平均值.
                editor: new Ext.form.NumberField({
                    allowBlank: false,
                    allowNegative: false,
                    style: 'text-align:left'
                ...
                sortable: false,
                groupable: false,
                renderer: function(v, params, record){		//值=Estimate * Rate
                    return Ext.util.Format.usMoney(record.data.estimate * record.data.rate);
                },
                dataIndex: 'cost',
                summaryType: 'totalCost',					//概要类型:自定义函数实现.当前组当前列所有行的求和
        ...
        view: new Ext.grid.GroupingView({
            ...
            hideGroupedColumn: true			//隐藏被分组的列
        }),

        plugins: summary,					//插件:概要

        tbar : [{
            text: 'Toggle',
            tooltip: 'Toggle the visibility of summary row',
            handler: function(){
            	summary.toggleSummaries();	//点击按钮,概要这一栏消失
            }
        },{
            text: '显示概要',
            tooltip: 'display the visibility of summary row',
            handler: function(){
            }
        }],
