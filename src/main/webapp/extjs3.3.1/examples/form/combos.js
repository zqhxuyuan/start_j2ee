Ext.onReady(function(){
    ...
    var store = new Ext.data.ArrayStore({
        fields: ['abbr', 'state', 'nick'],	//states.js中是静态的数据,没有字段名字.这里为数组中的每个元素赋值field
        data : Ext.exampledata.states 		// from states.js
    });
    var combo = new Ext.form.ComboBox({
        store: store,			//数据源
        displayField:'state',	//显示值
        mode: 'local',			//数据来自本地
        typeAhead: true,
        forceSelection: true,	//可以在下拉框中输入值,并自动选中输入项
        selectOnFocus:true,		
        triggerAction: 'all',
    ...
    //---------------------------------------------
    // 2. Simple arrays can be used directly as the store config.  1-dimensional arrays
