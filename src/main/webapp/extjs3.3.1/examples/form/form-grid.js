    ...
    // example of custom renderer function 列的颜色
    function change(val){
        if(val > 0){
...
Ext.onReady(function(){
	...
	//左边是一个Grid,右边是一个Form.两者构成了FormPanel
    var gridForm = new Ext.FormPanel({
        ...
        items: [{
            ...
            items: {
                ...
                ds: ds,
                cm: colModel,
                //行选择模式:一次选择一行. 当选中一行时,加载这条记录到表单里
                sm: new Ext.grid.RowSelectionModel({
                    singleSelect: true,
                    listeners: {
                        rowselect: function(sm, row, rec) {
                        	//company-form即右边的表单:将rec的数据加载到表单里
                ...
                border: true,
                listeners: {
                    viewready: function(g) { // Allow rows to be rendered.
                        g.getSelectionModel().selectRow(0);	//视图准备完毕,默认在表单里显示第一行的数据
                    } 
                }
            }
        },{
