...
scheduler.addEventNow = function (D, A, B) {
	...
	//添加event
	addorupdate(this.getEvent(this._drag_id));
};
	...
	if (this._drag_mode) {
		...
		if (!this._drag_pos || this._drag_pos.x != G.x || this._drag_pos.y != G.y) {
			...
			if (this._drag_mode == "create") {
				...
				//添加或者更新event
				//addorupdate(this._events[this._drag_id]);
			}
...
scheduler.locale = {
	date:{
		...
		icon_delete:"删除", 
		confirm_closing:"", 
		confirm_deleting:"您确定删除吗？", 
		section_description:"描述", section_time:"日期", 
...
scheduler._edit_stop_event = function (A, B) {
	...
	if (this._new_event) {
		//是否删除新建的event
		if (!B) {
			this.deleteEvent(A.id, true);//此函数中已添加了delEvent函数
		} else {
...
scheduler.addorupdate = function (ev) {
	var url = "";
	/*
	...
	if ($.browser.msie) {
		startdate = start_date.getYear();
		enddate = end_date.getYear();
	} else {
		startdate = start_date.getYear() + 1900;
        enddate = end_date.getYear() + 1900;
	}
	
	startdate += "-" + (start_date.getMonth() + 1) + "-" + start_date.getDate() + 
				" " +  start_date.getHours() + ":" + start_date.getMinutes() + ":" + start_date.getSeconds();

	enddate += "-" + (end_date.getMonth() + 1) + "-" + end_date.getDate() + 
				" " +  end_date.getHours() + ":" + end_date.getMinutes() + ":" + end_date.getSeconds();	
	*/			
	url += "event.do?method=addOrUpdateEvent&id=" + ev.id + "&startDate=" + ev.start_date + "&endDate=" + ev.end_date + 
		   "&text=" + encodeURI(ev.text) + "&details=" + encodeURI(ev.details);
		   alert(url);
//	new dhtmlxAjax();
//	dhtmlxAjax.getsync(url);
};
